---
order: 1
slug: hudson
year: 2022
cover: img/portfolio/hudson/1.webp
images:
  - img/portfolio/hudson/25.webp
  - img/portfolio/hudson/7.webp
  - img/portfolio/hudson/71.webp
  - img/portfolio/hudson/70.webp
  - img/portfolio/hudson/69.webp
  - img/portfolio/hudson/61.webp
  - img/portfolio/hudson/9.webp
  - img/portfolio/hudson/58.webp
  - img/portfolio/hudson/57.webp
  - img/portfolio/hudson/55.webp
  - img/portfolio/hudson/54.webp
  - img/portfolio/hudson/53.webp
  - img/portfolio/hudson/51.webp
  - img/portfolio/hudson/44.webp
  - img/portfolio/hudson/40.webp
  - img/portfolio/hudson/39.webp
  - img/portfolio/hudson/36.webp
  - img/portfolio/hudson/34.webp
  - img/portfolio/hudson/25.webp
  - img/portfolio/hudson/21.webp
  - img/portfolio/hudson/79.webp
en:
  title: "Luxury Residential Home"
  location: "Hudson, QC"
  address: "Hudson, QC"
  type: "Single-family Home"
  description: "This contemporary luxury residence features a modern architectural design with spacious interior layouts and large windows that provide natural light throughout the home. The property includes an open-concept kitchen with custom cabinetry, a large central island, and a secondary preparation kitchen. Interior spaces are finished with large-format tile flooring, modern lighting fixtures, and glass stair railings. The home also features multiple bathrooms with walk-in glass showers and contemporary finishes. The property includes a large driveway, landscaped grounds, and extensive outdoor space surrounding the residence."
  specs:
    - key: "Property Type"
      value: "Single-family Home"
    - key: "Levels"
      value: "2 floors + fully finished basement"
    - key: "Total Living Space"
      value: "4,465.98 sq ft"
    - key: "Bedrooms"
      value: "6"
    - key: "Bathrooms"
      value: "6"
    - key: "Garage"
      value: "4–6 car garage"
    - key: "Year Built"
      value: "2022"
fr:
  title: "Résidence résidentielle de luxe"
  location: "Hudson, QC"
  address: "Hudson, QC"
  type: "Résidence résidentielle"
  description: "Cette résidence contemporaine se distingue par une architecture moderne et des espaces intérieurs spacieux avec de grandes fenêtres apportant une abondante lumière naturelle. La propriété comprend une cuisine à aire ouverte avec armoires sur mesure, un grand îlot central ainsi qu'une cuisine de préparation séparée. Les espaces intérieurs sont dotés de planchers en tuiles grand format, d'un éclairage moderne et d'escaliers avec garde-corps en verre. La maison comprend également plusieurs salles de bain modernes avec douches vitrées et finitions contemporaines. La propriété dispose d'une grande entrée pavée, d'un aménagement paysager et d'un vaste terrain autour de la résidence."
  specs:
    - key: "Type de propriété"
      value: "Résidence résidentielle"
    - key: "Niveaux"
      value: "2 étages + sous-sol entièrement aménagé"
    - key: "Superficie habitable totale"
      value: "4 465,98 pi²"
    - key: "Chambres"
      value: "6"
    - key: "Salles de bain"
      value: "6"
    - key: "Garage"
      value: "Garage pour 4 à 6 voitures"
    - key: "Année de construction"
      value: "2022"
---
